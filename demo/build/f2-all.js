...
var catAuto = __webpack_require__(43);
...
var Smooth = __webpack_require__(45);
...
var Category = __webpack_require__(42);
...
/*#__PURE__*/
function (_Interaction) {
  _inheritsLoose(PieMove, _Interaction);

  var _proto = PieMove.prototype;
  ...
  function PieMove(cfg, chart) {
    var _this;

    _this = _Interaction.call(this, cfg, chart) || this;
    var lastMouseAngle, diffAngle;
    var animating = false;
    return _this;
  } // 获取基本参数


  _proto._getChartParam = function _getChartParam() {
  ...
  _proto._fromPointToAngle = function _fromPointToAngle(e) {
    var currentPoint = e.center;

    var center = this._getChartParam()[1];

    var currentAngle = Math.atan2(currentPoint.y - center.y, currentPoint.x - center.x) + Math.PI;
    return currentAngle;
  } // 获得箭头指向的元素
  ;

  _proto._getBottomShape = function _getBottomShape() {
    var container = this._getChartParam()[0];

    var pieShapes = container.get('children');
    var middleAngle, bottomShape;
    Util.each(pieShapes, function (s) {
      var startAngle = s.attr('startAngle');
      var endAngle = s.attr('endAngle'); // 找到底部的元素
  ...
  _proto._changeLegend = function _changeLegend() {
    var targetShape = this._getBottomShape()[1];
  ...
  _proto._interpolateNumber = function _interpolateNumber(a) {
    return function (t) {
      return a * t;
    };
  } // 根据最后一次的变化角度，计算持续时长
  ...
  _proto._handleDuration = function _handleDuration(angle) {
    var duration = Math.abs(Math.floor(1600 * angle));
  ...
  _proto._slideToMiddle = function _slideToMiddle(angle, animateConfig) {
    ...
    var diff = this._interpolateNumber(angleFromBottom);

    var _this$_getChartParam = this._getChartParam(),
        container = _this$_getChartParam[0],
        center = _this$_getChartParam[1];
  ...
  _proto._slideAnimation = function _slideAnimation(lastDiffAngle) {
    var _this3 = this;

    var diff = this._interpolateNumber(lastDiffAngle);

    var _this$_getChartParam2 = this._getChartParam(),
        container = _this$_getChartParam2[0],
        center = _this$_getChartParam2[1];
    ...
    container.animate().to({
      ...
      if (t === 1) {
        ...
        _this3._repaintChart(frameAngle);

        _this3._changeLegend();
      }
    }).onEnd(function () {
      var middleAngle = _this3._getBottomShape()[0];

      _this3._slideToMiddle(middleAngle, {
        duration: 850,
        easing: 'backOut'
      });
    });
  ...
  _proto.process = function process(ev) {
    var newMouseAngle = this._fromPointToAngle(ev);

    this.diffAngle = newMouseAngle - this.lastMouseAngle; // 更新上一次移动的角度为本次移动起始的角度
